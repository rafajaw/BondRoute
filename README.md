...
A cryptographic bond is a staked commitment to an action. Intent is hidden until execution. Abandonment forfeits stake.
...
**The fix:** BondRoute makes user actions invisible until execution, and makes abandoning actions expensive.
...
Private mempools and relayers don't fix this — they introduce trust assumptions and move extraction one level up.

### What BondRoute Does

BondRoute gives your protocol two properties that eliminate these attacks:

**Hidden intent** - All actions flow through a single router. Users commit to an action (hidden as a hash), wait at least one block, then reveal and execute. Attackers see a bond was created — but not which protocol, which function, or what parameters.

**Binding commitment** - Every bond locks stake defined by your protocol. Users who execute get their stake back. Bonds that expire without execution forfeit their stake permanently.

The result: attackers can't watch and react (intent is hidden), and they can't spray-and-pray (each attempt costs real money).
...
|---------------|----------------|
| Bot sees your user's pending swap | Swap intent hidden until execution |
| Bot frontruns or sandwiches | Nothing to frontrun — intent revealed only at execution |
...
| Without bonds | With BondRoute |
|---------------|----------------|
| Bidder commits to $100, $200... $900 | Each bid requires stake |
| Reveals only the winner | Eight abandoned bids forfeit stake |
| Gaming auctions is free | Gaming auctions is expensive |
...
| Bot B sees pending tx, frontruns with higher gas | Nothing to frontrun — position unknown until reveal |
...
**Trustless design.** Everything lives on-chain: protocol discovery, protected selector queries, constraint lookups. No off-chain dependencies.
...
**Commit under review:** `988061e83667112bc19c22ba9bed16cdf64bd638`
...
<details>
<summary><strong>What's the business case for integrating?</strong></summary>

It prevents execution-time value extraction, turning a known user risk into a competitive advantage.
</details>

<details>
<summary><strong>Does this lock us into a vendor relationship?</strong></summary>
...
<details>
<summary><strong>Do I have to protect all functions?</strong></summary>
...
After 111 days, unexecuted bonds can be liquidated by the collector. The goal of stakes is deterrence, not revenue — under normal use, users execute within the window and get their stakes back.

Forfeited stakes are claimable only by the collector address. BondRoute core does not fix how the collector must use them; it can be a simple sweeper or a more complex distribution contract. Any such policy is implemented outside the core primitive and does not affect bond semantics.
</details>

<details>
<summary><strong>Why 111 days?</strong></summary>
...
~40-66k gas depending on stake type (zero/native/ERC20) and protocol access pattern (warm/cold). Typically a few cents on Ethereum and even lower on other EVM chains.
</details>
...
<details>
<summary><strong>What about flash loans?</strong></summary>

Flash loans work perfectly. Execute bonds inside flash loan callbacks — borrowed capital funds executions, atomicity preserved.
</details>

<details>
<summary><strong>Can I use native ETH for stakes or fundings?</strong></summary>
