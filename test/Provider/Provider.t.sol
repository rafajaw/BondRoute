...
import { Provider, Forbidden, ProtocolAnnounced, Airdrop } from "@BondRoute/Provider.sol";
...
contract ProviderHarness is Provider {
    ...
    function exposed_set_collector( address collector ) external
    {
...
contract ProviderTest is Test {
    ...
    address public constant COLLECTOR  =  address(0x3333);

    function setUp() public
    {
        provider_harness  =  new ProviderHarness();
        provider_harness.exposed_set_collector( COLLECTOR );
    ...
    function test_airdrop_native_token_transfers_to_collector_and_emits_event() public
    {
        address protocol  =  address(0x9999);
        uint256 amount    =  1 ether;
        string memory message  =  "Launch signal";

        uint256 collector_balance_before  =  COLLECTOR.balance;

        vm.expectEmit( true, true, true, true );
        emit Airdrop( address(this), protocol, address(0), amount, message );

        provider_harness.airdrop{ value: amount }( protocol, IERC20(address(0)), amount, message );

        assertEq( COLLECTOR.balance, collector_balance_before + amount, "Collector should receive native token" );
    }

    function test_airdrop_erc20_transfers_to_collector_and_emits_event() public
    {
        address protocol  =  address(0x9999);
        uint256 amount    =  1000e6;
        string memory message  =  "ERC20 signal";

        usdc.mint( address(this), amount );
        usdc.approve( address(provider_harness), amount );

        uint256 collector_balance_before  =  usdc.balanceOf( COLLECTOR );

        vm.expectEmit( true, true, true, true );
        emit Airdrop( address(this), protocol, address(usdc), amount, message );

        provider_harness.airdrop( protocol, usdc, amount, message );

        assertEq( usdc.balanceOf( COLLECTOR ), collector_balance_before + amount, "Collector should receive ERC20" );
    }
    ...
    function test_airdrop_reverts_on_native_amount_mismatch() public
    {
        ...
        // Case 1: Native token with msg.value != amount.
        vm.expectRevert( abi.encodeWithSelector( NativeAmountMismatch.selector, 0.5 ether, 1 ether ) );
        provider_harness.airdrop{ value: 0.5 ether }( protocol, IERC20(address(0)), 1 ether, "test" );

        // Case 2: ERC20 with msg.value > 0.
        vm.expectRevert( abi.encodeWithSelector( NativeAmountMismatch.selector, 1 ether, 0 ) );
        provider_harness.airdrop{ value: 1 ether }( protocol, usdc, 1000e6, "test" );

        // Case 3: amount=0 with msg.value > 0.
        vm.expectRevert( abi.encodeWithSelector( NativeAmountMismatch.selector, 1 ether, 0 ) );
        provider_harness.airdrop{ value: 1 ether }( protocol, IERC20(address(0)), 0, "test" );
    }

    function test_airdrop_message_only_emits_event_without_transfer() public
    {
        address protocol  =  address(0x9999);
        string memory message  =  "Micro-airdrop announcement";

        uint256 collector_balance_before  =  COLLECTOR.balance;

        vm.expectEmit( true, true, true, true );
        emit Airdrop( address(this), protocol, address(0), 0, message );

        provider_harness.airdrop( protocol, IERC20(address(0)), 0, message );

        assertEq( COLLECTOR.balance, collector_balance_before, "No transfer should occur for amount=0" );
    }
    ...
    function test_airdrop_truncates_long_message() public
    {
        ...
        // Create message longer than MAX_MESSAGE_LENGTH (280 bytes).
        bytes memory long_bytes  =  new bytes(300);
        for(  uint i = 0  ;  i < 300  ;  i++  )  long_bytes[i]  =  "a";
        string memory long_message  =  string(long_bytes);
        assertGt( bytes(long_message).length, MAX_MESSAGE_LENGTH, "Test message should exceed max length" );

        // Build expected truncated message (first 280 bytes).
        bytes memory truncated_bytes  =  new bytes(MAX_MESSAGE_LENGTH);
        for(  uint i = 0  ;  i < MAX_MESSAGE_LENGTH  ;  i++  )  truncated_bytes[i]  =  "a";
        string memory expected_truncated  =  string(truncated_bytes);
        assertEq( bytes(expected_truncated).length, MAX_MESSAGE_LENGTH, "Expected truncated message should be exactly MAX_MESSAGE_LENGTH" );

        vm.expectEmit( true, true, true, true );
        emit Airdrop( address(this), protocol, address(0), amount, expected_truncated );

        provider_harness.airdrop{ value: amount }( protocol, IERC20(address(0)), amount, long_message );
    }
    ...
    function test_transfer_funding_reverts_on_zero_address() public
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[ 0 ]  =  TokenAmount({ token: usdc, amount: 1000e6 });
        ...
        vm.expectRevert( abi.encodeWithSelector( Invalid.selector, "to", 0 ) );
        vm.prank( address(mock_protocol) );
        provider_harness.transfer_funding( address(0), usdc, 1000e6, context );
    }
