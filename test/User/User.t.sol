...
import { User, NativeAmountMismatch, CommitmentMismatch, InvalidSignature, EIP712Domain } from "@BondRoute/User.sol";
...
contract UserTest is Test {
    ...
    function test_create_bond_reverts_on_zero_commitment_hash() public
    {
        ...
        vm.expectRevert( abi.encodeWithSelector( CommitmentMismatch.selector, bytes32(0), block.chainid, address(usdc), 100e6 ) );

        vm.prank( USER );
        user_harness.create_bond( bytes32(0), stake );
    }

    function test_create_bond_reverts_on_invalid_commitment_hash_wrong_stake() public
    {
        TokenAmount memory original_stake  =  TokenAmount({ token: usdc, amount: 100e6 });
        TokenAmount memory wrong_stake     =  TokenAmount({ token: usdc, amount: 200e6 });

        ExecutionData memory execution_data  =  _create_test_execution_data( original_stake, 101 );

        bytes32 commitment_hash  =  user_harness.__OFF_CHAIN__calc_commitment_hash( USER, execution_data );

        vm.startPrank( USER );
        usdc.approve( address(user_harness), 200e6 );

        vm.expectRevert( abi.encodeWithSelector( CommitmentMismatch.selector, commitment_hash, block.chainid, address(usdc), 200e6 ) );
        user_harness.create_bond( commitment_hash, wrong_stake );
        vm.stopPrank();
    }

    function test_create_bond_reverts_on_invalid_commitment_hash_corrupted() public
    {
        TokenAmount memory stake  =  TokenAmount({ token: usdc, amount: 100e6 });
        ExecutionData memory execution_data  =  _create_test_execution_data( stake, 102 );

        bytes32 commitment_hash  =  user_harness.__OFF_CHAIN__calc_commitment_hash( USER, execution_data );

        // Corrupt the hash.
        bytes32 corrupted_hash  =  bytes32( uint256(commitment_hash) ^ 0x100 );

        vm.expectRevert( abi.encodeWithSelector( CommitmentMismatch.selector, corrupted_hash, block.chainid, address(usdc), 100e6 ) );

        vm.prank( USER );
        user_harness.create_bond( corrupted_hash, stake );
    }
    ...
    function test_execute_bond_as_reverts_on_invalid_signature() public
    {
        ...
        vm.expectRevert( abi.encodeWithSelector( InvalidSignature.selector, user_from_key, digest, invalid_signature ) );
    ...
    function test_execute_bond_as_reverts_on_wrong_signer() public
    {
        ...
        vm.expectRevert( abi.encodeWithSelector( InvalidSignature.selector, user_from_key, digest, signature ) );
