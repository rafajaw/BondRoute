...
contract HashLibTest is Test {
    ...
    function test_calc_commitment_hash_has_caffe0_prefix() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        // Top 3 bytes should be 0xCAFFE0.
        uint256 prefix  =  uint256(hash) >> 232;

        assertEq( prefix, 0xCAFFE0, "Commitment hash should have 0xCAFFE0 prefix" );
    }

    function test_calc_commitment_hash_has_zero_padding() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        // Bytes 3-8 (6 bytes after prefix) should be zeros.
        // Layout: [3 bytes prefix][6 bytes zeros][21 bytes hash][2 bytes sentinel].
        uint256 padding  =  ( uint256(hash) >> 184 ) & 0xFFFFFFFFFFFF;

        assertEq( padding, 0, "Commitment hash should have 6 zero bytes after prefix" );
    }

    function test_calc_commitment_hash_different_stake_different_hash() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        ExecutionData memory exec_data_2  =  ExecutionData({
            fundings: fundings,
            stake: TokenAmount({ token: token_b, amount: 50e18 }),
            salt: 12345,
            protocol: mock_protocol,
            call: abi.encodeWithSignature("someFunction()")
        });

        ExecutionData memory exec_data_3  =  ExecutionData({
            fundings: fundings,
            stake: TokenAmount({ token: token_a, amount: 100e18 }),
            salt: 12345,
            protocol: mock_protocol,
            call: abi.encodeWithSignature("someFunction()")
        });
        ...
        bytes32 hash_1  =  HashLib.calc_commitment_hash( address(0x1234), address(bondroute), exec_data_1 );
        bytes32 hash_2  =  HashLib.calc_commitment_hash( address(0x1234), address(bondroute), exec_data_2 );
        bytes32 hash_3  =  HashLib.calc_commitment_hash( address(0x1234), address(bondroute), exec_data_3 );

        assertTrue( hash_1 != hash_2, "Different stake token should produce different commitment hash" );
        assertTrue( hash_1 != hash_3, "Different stake amount should produce different commitment hash" );
    }


    // ━━━━  is_valid_commitment_hash VALIDATION  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    function test_is_valid_commitment_hash_accepts_valid_hash() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });

        TokenAmount memory stake  =  TokenAmount({ token: token_a, amount: 50e18 });
        ...
        bytes32 hash  =  HashLib.calc_commitment_hash( address(0x1234), address(bondroute), exec_data );
        bool is_valid  =  HashLib.is_valid_commitment_hash( hash, stake );

        assertTrue( is_valid, "Valid commitment hash should be accepted" );
    }

    function test_is_valid_commitment_hash_rejects_wrong_stake_token() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });

        TokenAmount memory original_stake  =  TokenAmount({ token: token_a, amount: 50e18 });
        TokenAmount memory wrong_stake     =  TokenAmount({ token: token_b, amount: 50e18 });
        ...
        bytes32 hash  =  HashLib.calc_commitment_hash( address(0x1234), address(bondroute), exec_data );
        bool is_valid  =  HashLib.is_valid_commitment_hash( hash, wrong_stake );

        assertTrue( is_valid == false, "Wrong stake token should be rejected" );
    }

    function test_is_valid_commitment_hash_rejects_wrong_stake_amount() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });

        TokenAmount memory original_stake  =  TokenAmount({ token: token_a, amount: 50e18 });
        TokenAmount memory wrong_stake     =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        assertTrue( is_valid == false, "Wrong stake amount should be rejected" );
    }

    function test_is_valid_commitment_hash_rejects_corrupted_hash() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        // Corrupt one bit in the hash portion (not the sentinel).
        bytes32 corrupted_hash  =  bytes32( uint256(hash) ^ (1 << 20) );

        bool is_valid  =  HashLib.is_valid_commitment_hash( corrupted_hash, stake );

        assertTrue( is_valid == false, "Corrupted hash should be rejected" );
    }

    function test_is_valid_commitment_hash_rejects_zero_hash() public view
    {
        TokenAmount memory stake  =  TokenAmount({ token: token_a, amount: 50e18 });

        bool is_valid  =  HashLib.is_valid_commitment_hash( bytes32(0), stake );

        assertTrue( is_valid == false, "Zero hash should be rejected" );
    }

    function test_is_valid_commitment_hash_rejects_corrupted_sentinel() public view
    {
        TokenAmount[] memory fundings  =  new TokenAmount[](1);
        fundings[0]  =  TokenAmount({ token: token_a, amount: 100e18 });
        ...
        // Corrupt the sentinel (low 2 bytes).
        bytes32 corrupted_hash  =  bytes32( uint256(hash) ^ 0x0001 );

        bool is_valid  =  HashLib.is_valid_commitment_hash( corrupted_hash, stake );

        assertTrue( is_valid == false, "Corrupted sentinel should be rejected" );
    }
