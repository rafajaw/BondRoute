...
import { TokenAmount } from "./integrations/BondRouteProtected.sol";
...
/**
 * @title Storage
 * @notice Core state: bond records, collector role, and transient execution context
 * @dev Bond state packed into single slot. Transient storage tracks held funds during execution.
 */
abstract contract Storage is ReentrancyLock {
    ...
    // Two-step transfer prevents accidental loss of collector role to an invalid or inaccessible address.
    ...
    // *GAS SAVING*  -  XOR-based slot addressing saves gas per access vs mapping (avoids keccak256 for slot calculation).
    //                  Single storage slot per bond. Key = `keccak256(commitment_hash || stake.token || stake.amount)`.
    ...
    bytes32 private constant BONDS_BASE_SLOT  =  bytes32(uint256(keccak256("BondRoute.bonds.base_slot")) - 1);

    function _read_bond_slot( bytes32 bond_key ) private view returns ( uint256 value )
    {
        bytes32 base  =  BONDS_BASE_SLOT;
        assembly ("memory-safe") {
            value  :=  sload( xor( bond_key, base ) )
        }
    }

    function _write_bond_slot( bytes32 bond_key, uint256 value ) private
    {
        bytes32 base  =  BONDS_BASE_SLOT;
        assembly ("memory-safe") {
            sstore( xor( bond_key, base ), value )
        }
    }
    ...
    function _create_bond_internal( bytes32 commitment_hash, TokenAmount memory stake, uint256 amount_received ) internal
    {
        ...
        if(  _read_bond_slot( bond_key ) != 0  )  revert BondAlreadyExists( commitment_hash, address(stake.token), stake.amount );

        // Calculate delta: negative = fee taken, positive = bonus received
        int128 stake_received_delta;
        ...
        unchecked
        {
            if(  stake.amount > amount_received  )
            {
                ...
                if(  loss > uint256(uint128(type(int128).max))  )  revert UnsupportedStake( stake.amount, amount_received );
            ...
            else if(  amount_received > stake.amount  )
            {
                ...
                if(  gain > uint256(uint128(type(int128).max))  )  revert UnsupportedStake( stake.amount, amount_received );
        ...
        _write_bond_slot( bond_key, packed_value );
    }
    ...
    internal view returns ( BondInfo memory bond_info, bytes32 bond_key, uint256 packed_value )
    {
        ...
        packed_value    =  _read_bond_slot( bond_key );
        if(  packed_value == 0  )  revert BondNotFound( commitment_hash, address(stake.token), stake.amount );
    ...
    {
        _write_bond_slot( bond_key, previous_packed_value | ( uint256(uint8(new_status)) << 248 ) );
    }
