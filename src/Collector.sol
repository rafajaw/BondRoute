...
abstract contract Collector is Provider {
     ...
     * @dev Reverts with `Invalid("collector", 0)` if collector is zero address
     */
     ...
     *
     * @dev Emits `NewCollectorAppointed(new_collector)` upon successful appointment.
     *
     * @dev Reverts with `Unauthorized` if caller is not current collector.
     * @dev Reverts with `Invalid("new_collector", 0)` if new_collector is zero address.
     */
     ...
     *
     * @dev Emits `NewCollector(collector)` upon successful role claim.
     *
     * @dev Reverts with `Unauthorized` if caller is not the pending collector.
     */
     ...
     *
     * @dev Emits `AirdropReceived(sender, token, amount_received, message)` upon successful airdrop.
     *
     ...
     * @dev Reverts with `Reentrancy` if reentering airdrop functions.
     */
    ...
    /**
     * @notice Gas-efficient alternative to `airdrop()` for tokens that mint directly to BondRoute (caller IS the token)
     * @param amount Amount sent to BondRoute - must be minted or transferred before calling
     * @param message Optional string message cast to bytes32 - pass `bytes32(0)` for silent mode - no event emitted
     *
     ...
     *
     * @dev Emits `AirdropCredited(msg.sender, amount, message)` if `message != bytes32(0)`.
     *
     ...
     *
     * @dev Emits `AirdropClaimed(token, claimable, recipient)` for each token with claimable airdrops > 1 wei.
     *
     * @dev Reverts with `Unauthorized` if caller is not the collector.
