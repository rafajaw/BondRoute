...
string constant REASON_INVALID_PREFIX          =   "Invalid prefix";
string constant REASON_TokenAmount_MISMATCH    =   "TokenAmount mismatch";
...
library ValidationLib {
    ...
    function _does_support_bondroute_and_selector( IBondRouteProtected target_contract, bytes4 target_selector ) internal view returns ( bool )
    {
        ...
        if(  gasleft() < MIN_GAS_FOR_SELECTOR_QUERY  )  revert PossiblyBondFarming({ reason: OUT_OF_GAS_OR_UNSPECIFIED_FAILURE, additional_info: 0 });
        ...
        // *NOTE*  -  No length cap on selectors. OOG here is no different from OOG anywhere else in execution — user retries with more gas.
        unchecked
        {
            uint ptr;
            uint end;
    ...
    function revert_if_possibly_bond_farming( bytes memory call_output ) internal pure
    {
        ...
        {
            revert PossiblyBondFarming({ reason: OUT_OF_GAS_OR_UNSPECIFIED_FAILURE, additional_info: 0 });
        }
        ...
        if(  call_output.length >= 4  )
        {
            ...
            if(  error_selector == TransferFailed.selector  )
            {
                ...
                revert PossiblyBondFarming({ reason: TRANSFER_FAILED, additional_info: bytes32(uint256(uint160(token))) });
            }
            ...
            {
                revert PossiblyBondFarming({ reason: REENTRANCY, additional_info: 0 });
            }
    ...
    function validate_typed_string_prefix( string memory typed_string ) internal pure
    {
        ...
        //
        // *SECURITY*  -  No validation after "protocol," — the next argument is the "call" which protocol defines freely.
        //                Extra arguments beyond "call" cannot be exploited: `calc_struct_hash_for_execute_bond_as()` uses
        //                a fixed field count (5 + type_hash). Protocol only provides the inner calldata_hash, not the
        //                final struct hash. If protocol's type string has extra fields signature verification fails.

        bytes memory typed_bytes  =  bytes(typed_string);

        if(  typed_bytes.length < 96  )  revert InvalidTypedString( typed_string, REASON_INVALID_PREFIX );
        ...
        bytes32 mask3  =  0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000000000000000000000;  // Mask first 21 bytes for word3.
        bytes32 xor_result  =  (word1 ^ bytes32("ExecuteBondAs(TokenAmount[] fund"))
                            |  (word2 ^ bytes32("ings,TokenAmount stake,uint256 s"))
                            |  ((word3 & mask3) ^ bytes32("alt,address protocol,"));

        if(  xor_result != 0  )  revert InvalidTypedString( typed_string, REASON_INVALID_PREFIX );
    }
    ...
    function validate_TokenAmount_definition( string memory typed_string, uint256 TokenAmount_offset ) internal pure
    {
        ...
        if(  TokenAmount_offset == 0  )  revert InvalidTypedString( typed_string, REASON_TokenAmount_MISMATCH );
        ...
        // Expected: ")TokenAmount(address token,uint256 amount)"
        bytes32 mask10  =  0xFFFFFFFFFFFFFFFFFFFF00000000000000000000000000000000000000000000;  // Mask first 10 bytes for word2.
        bytes32 xor_result  =  (word1 ^ bytes32(")TokenAmount(address token,uint2"))
                            |  ((word2 & mask10) ^ bytes32("56 amount)"));

        if(  xor_result != 0  )  revert InvalidTypedString( typed_string, REASON_TokenAmount_MISMATCH );
    }
}
